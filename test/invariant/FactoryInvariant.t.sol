...
/// @notice Invariant suite for {BLOKCAmbassadorFactory}, driven by the
///         pre-existing {FactoryHandler}. The handler creates accounts for
///         a fixed ambassador pool via both overloads and re-creates for
///         existing ambassadors to hammer the idempotent short-circuit.
///
/// @dev    Properties asserted after every handler call:
///           - Registry length equals the number of distinct ambassadors.
///           - Every ambassador maps to a stable, non-zero account that
///             equals both the handler's last-returned address and the
///             factory's own deterministic prediction.
///           - Every registered account is owned by an ambassador that maps
...
contract FactoryInvariant is StdInvariant, Test {
    ...
    ///         ambassador — idempotent re-creates never grow it.
    function invariant_factory_registryLengthMatchesUnique() public view {
        assertEq(
            factory.getAllAccounts().length, handler.g_uniqueAmbassadors(), "registry length != unique ambassadors"
        );
    }

    /// @notice Each seen ambassador maps to a stable, non-zero account that
    ///         matches both the handler's record and the factory's
    ///         deterministic prediction.
    function invariant_factory_accountStableAndPredicted() public view {
        uint256 n = handler.seenAmbassadorsLength();
        for (uint256 i; i < n; ++i) {
            address amb = handler.g_seenAmbassadors(i);
            address acct = factory.accountOf(amb);
            assertTrue(acct != address(0), "seen ambassador has no account");
            assertEq(acct, handler.g_lastReturned(amb), "account address drifted from handler record");
            assertEq(acct, factory.predictAmbassadorAccount(amb), "account != deterministic prediction");
            assertTrue(factory.holdsAccount(amb), "holdsAccount false for seen ambassador");
        }
    }

    /// @notice Every account in the registry is owned by an ambassador that
    ...
    function invariant_factory_everyAccountOwnedAndBacklinked() public view {
        ...
        for (uint256 i; i < accounts.length; ++i) {
            address owner = BLOKCAmbassadorAccount(accounts[i]).ambassador();
